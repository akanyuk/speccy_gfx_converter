...
class ZXGFX {
    ...
    private $options = array(
        'forceOneFrame' => false,        // Force remove second frame in GIF-images
    ...
    private function generateImage() {
        ...
        if ($this->output_type == 'gif') {
            ...
            if ($frame2 && !$this->options['forceOneFrame']) {
                ...
                $gif_frames[] = ob_get_contents();
                ob_end_clean();
                $this->destroyFrame($frame2);

                // GifCreator errors suppressing
                $cur_e = error_reporting(0);
                $GIF = new GifCreator\GifCreator();
                try {
                    $GIF->create($gif_frames, array($this->animation_delay, $this->animation_delay));
                } catch (Exception $e) {
                    exit($e->getMessage());
                }
                $result_img = $GIF->getGif();
    ...
    private function createGigascreenFrame() {
        $im = @imagecreatetruecolor(256, 192) or die("Cannot Initialize new GD image stream");
        imagefill($im, 0, 0, 0);
    ...
    private function createMgsFrame() {
        $im = @imagecreatetruecolor(256, 192) or die("Cannot Initialize new GD image stream");
        imagefill($im, 0, 0, 0);
    ...
    private function create3ColorFrame() {
        $im = @imagecreatetruecolor(256, 192) or die("Cannot Initialize new GD image stream");
        imagefill($im, 0, 0, 0);
    ...
    /**
     * Resizing generated image and adding border
     * @param $img GdImage
     * @return mixed
     */
    private function finalizeFrame($img) {
        ...
        if ($this->output_scale != 1) {
            imagecopyresampled($frame, $img, $image_left, $image_top, 0, 0, $this->output_scale * 256, $this->output_scale * 192, 256, 192);
        } else {
            imagecopy($frame, $img, $image_left, $image_top, 0, 0, 256, 192);
        }
    ...
    /**
     * Generating image and force download
    ...
    /**
     * Generating image and save it on server
     * @param $path string
     * @return false|string
     */
    function save($path) {
